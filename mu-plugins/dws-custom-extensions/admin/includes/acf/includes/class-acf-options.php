...
final class ACF_Options extends DWS_Functionality_Template {
	 ...
	 * @version 1.0.0
	 *
	 * @var     string      MENU_PAGES_SLUG_PREFIX      The proper prefix of the slug of all ACF options pages of this plugin.
	 */
	...
	public function add_sub_pages() {
		...
		foreach($sub_pages as $sub_page) {
			if (!isset($sub_page['menu_title'],$sub_page['menu_slug'])) { continue; }
	...
	private function add_groups($groups, $location) {
		foreach ($groups as $group) {
			if (!isset($group['key'], $group['title'], $group['fields'])) { continue; }
	...
	private function add_fields($fields) {
		...
		global $wp_actions; unset( $wp_actions[ 'acf/include_fields' ] );
